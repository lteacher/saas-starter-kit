...
export const createAuditLog = async (db: Db, logData: {
  userId: string;
  action: string;
  resource: string;
  resourceId?: string;
...
export const listAuditLogs = async (db: Db, options: {
  limit?: number;
  offset?: number;
  userId?: string;
  resource?: string;
  ...
  const [logs, total] = await Promise.all([
    db.collection<AuditLog>('audit-logs')
      .find(filter)
      .sort({ createdAt: -1 })
      .skip(offset)
      .limit(limit)
...
export const getUserAuditLogs = async (db: Db, userId: string, options: {
  limit?: number;
  offset?: number;
} = {}): Promise<{ logs: AuditLog[]; total: number }> => {
...
export const getResourceAuditLogs = async (db: Db, resource: string, resourceId?: string, options: {
  limit?: number;
  offset?: number;
} = {}): Promise<{ logs: AuditLog[]; total: number }> => {
