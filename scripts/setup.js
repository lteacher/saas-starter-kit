...
function toTitleCase(kebabName) {
  return kebabName.split('-').map(word => 
...
function replaceInContent(content, projectName) {
  ...
  return content
    ...
    .replace(/"description": "SaaS Starter Kit - Qwik Frontend"/, `"description": "${titleName} - Qwik Frontend"`);
}
...
function updateFilesRecursively(dir, projectName) {
  ...
  for (const item of items) {
    ...
    if (stat.isDirectory()) {
      ...
      // Update text files
      const textExtensions = ['.json', '.md', '.ts', '.tsx', '.js', '.jsx', '.html', '.txt', '.toml', '.env', '.example'];
...
function main() {
  ...
  try {
    copyFileSync(join(projectRoot, 'services/api/.env.example'), join(projectRoot, 'services/api/.env'));
